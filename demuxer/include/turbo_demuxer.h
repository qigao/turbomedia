 ...
 * 初始化 Demuxer 注册表
 */
TURBO_MEDIA_API void turbo_demuxer_registry_init(void);

/**
 * 关闭 Demuxer 注册表
 */
TURBO_MEDIA_API void turbo_demuxer_registry_shutdown(void);

/**
 * 注册 Demuxer
 */
TURBO_MEDIA_API int turbo_demuxer_register(const turbo_demuxer_ops_t *ops);

/**
 * 根据名称查找 Demuxer
 */
TURBO_MEDIA_API const turbo_demuxer_ops_t *turbo_demuxer_find_by_name(const char *name);

/**
 * 根据文件扩展名查找 Demuxer
 */
TURBO_MEDIA_API const turbo_demuxer_ops_t *turbo_demuxer_find_by_extension(const char *ext);

/**
 * 自动探测格式
 */
TURBO_MEDIA_API const turbo_demuxer_ops_t *turbo_demuxer_probe(const uint8_t *data, size_t size);
 ...
 */
TURBO_MEDIA_API turbo_demuxer_t *turbo_demuxer_create(const turbo_demuxer_config_t *config);

/**
 * 创建指定格式的 Demuxer 实例
 */
TURBO_MEDIA_API turbo_demuxer_t *turbo_demuxer_create_by_name(const char *name,
                                                        const turbo_demuxer_config_t *config);

/**
 * 销毁 Demuxer 实例
 */
TURBO_MEDIA_API void turbo_demuxer_destroy(turbo_demuxer_t *demuxer);

/**
 * 打开容器
 */
TURBO_MEDIA_API int turbo_demuxer_open(turbo_demuxer_t *demuxer);

/**
 * 读取数据包
 */
TURBO_MEDIA_API int turbo_demuxer_read_packet(turbo_demuxer_t *demuxer,
                                        turbo_demuxer_packet_t *packet);

/**
 * 释放数据包
 */
TURBO_MEDIA_API void turbo_demuxer_free_packet(turbo_demuxer_packet_t *packet);

/**
 * 定位
 */
TURBO_MEDIA_API int turbo_demuxer_seek(turbo_demuxer_t *demuxer, int64_t timestamp_ms, int flags);

/**
 * 获取流数量
 */
TURBO_MEDIA_API int turbo_demuxer_get_stream_count(turbo_demuxer_t *demuxer);

/**
 * 获取流信息
 */
TURBO_MEDIA_API int turbo_demuxer_get_stream_info(turbo_demuxer_t *demuxer,
                                            int stream_index,
 ...
 * 获取元数据
 */
TURBO_MEDIA_API int turbo_demuxer_get_metadata(turbo_demuxer_t *demuxer,
