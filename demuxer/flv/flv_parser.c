...
static int flv_parser_audio(flv_audio_tag_header_t* audio, const uint8_t* data, size_t bytes, uint32_t timestamp, flv_parser_handler handler, void* param)
{
	if (FLV_SEQUENCE_HEADER == audio->av_packet)
		return handler(param, FLV_AUDIO_AAC == audio->codec_id ? FLV_AUDIO_ASC : FLV_AUDIO_OPUS_HEAD, data, bytes, timestamp, timestamp, 0);
	else
		return handler(param, audio->codec_id, data, bytes, timestamp, timestamp, 0);
}

static int flv_parser_video(flv_video_tag_header_t* video, const uint8_t* data, size_t bytes, uint32_t timestamp, flv_parser_handler handler, void* param)
{
	if (FLV_VIDEO_H264 == video->codec_id || FLV_VIDEO_H265 == video->codec_id || FLV_VIDEO_H266 == video->codec_id || FLV_VIDEO_AV1 == video->codec_id)
	{
		if (FLV_SEQUENCE_HEADER == video->av_packet)
		{
			return handler(param, FLV_VIDEO_CODEC_NAME(video->codec_id), data, bytes, timestamp, timestamp, 0);
		}
		else if (FLV_AVPACKET == video->av_packet)
		{
			return handler(param, video->codec_id, data, bytes, timestamp + video->cts, timestamp, (FLV_VIDEO_KEY_FRAME == video->keyframe) ? 1 : 0);
		}
		else if (FLV_END_OF_SEQUENCE == video->av_packet)
		{
	...
	{
		// Video frame data
		return handler(param, video->codec_id, data, bytes, timestamp, timestamp, (FLV_VIDEO_KEY_FRAME == video->keyframe) ? 1 : 0);
	}
}
	...
	for (n = r = 0; bytes > 0 && n >= 0 && 0 == r; data += n, bytes -= n)
	{
		...
		case FLV_HEADER:
			...
			if (FLV_HEADER_SIZE == parser->bytes)
			{
				...
				if (parser->header.data_offset < 9 || parser->header.data_offset > sizeof(parser->ptr))
					return -1;
				parser->header.data_offset -= 9;
				parser->state = parser->header.data_offset > 0 ? FLV_HEADER_OFFSET : FLV_PREVIOUS_SIZE;
				parser->bytes = 0;
			}
			break;

		case FLV_HEADER_OFFSET:
			n = flv_parser_append(parser, data, bytes, parser->header.data_offset);
			if (parser->header.data_offset == (uint32_t)parser->bytes)
			{
				parser->bytes = 0;
