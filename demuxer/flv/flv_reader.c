...
static int flv_read_header(struct flv_reader_t* flv)
{
	...
	assert(h.data_offset >= FLV_HEADER_SIZE && h.data_offset < FLV_HEADER_SIZE + 4096);
	for(n = (int)(h.data_offset - FLV_HEADER_SIZE); n > 0 && n < 4096; n -= sizeof(data))
...
int flv_reader_read(void* p, int* tagtype, uint32_t* timestamp, size_t* taglen, void* buffer, size_t bytes)
{
	...
	*timestamp = tag.timestamp;
    ...
    assert(0 == tag.stream_id); // StreamID Always 0
