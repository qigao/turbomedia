...
static int flv_demuxer_audio(struct flv_demuxer_t* flv, const uint8_t* data, int bytes, uint32_t timestamp)
{
	...
	if (FLV_AUDIO_AAC == audio.codec_id)
	{
		...
		else if (FLV_AVPACKET == audio.av_packet)
		{
	...
	else if (FLV_AUDIO_OPUS == audio.codec_id)
	{
		if (FLV_SEQUENCE_HEADER == audio.av_packet)
		{
		...
		{
			return flv->handler(flv->param, audio.codec_id, data + n, bytes - n, timestamp, timestamp, 0);
		}
	}
	else if (FLV_AUDIO_FLAC == audio.codec_id)
	{
		if (FLV_SEQUENCE_HEADER == audio.av_packet)
		{
	...
	else
	{
		...
		if (FLV_AVPACKET == audio.av_packet)
			return flv->handler(flv->param, audio.codec_id, data + n, bytes - n, timestamp, timestamp, 0);
	}
...
static int flv_demuxer_video(struct flv_demuxer_t* flv, const uint8_t* data, int bytes, uint32_t timestamp)
{
	...
	if (FLV_VIDEO_H264 == video.codec_id)
	{
	...
	else if (FLV_VIDEO_H265 == video.codec_id)
	{
		if (FLV_SEQUENCE_HEADER == video.av_packet)
		{
		...
		{
			assert(video.av_packet >= FLV_PACKET_TYPE_CODED_FRAMES_X && video.av_packet <= FLV_PACKET_TYPE_MULTITRACK);
			return 0;
		}
	}
	else if (FLV_VIDEO_H266 == video.codec_id)
	{
		if (FLV_SEQUENCE_HEADER == video.av_packet)
		{
	...
	else if (FLV_VIDEO_AV1 == video.codec_id)
	{
		if (FLV_SEQUENCE_HEADER == video.av_packet)
		{
	...
	else if (FLV_VIDEO_AVS3 == video.codec_id)
	{
		if (FLV_SEQUENCE_HEADER == video.av_packet)
		{
	...
	{
		// Video frame data
		return flv->handler(flv->param, video.codec_id, data + n, bytes - n, timestamp + video.cts, timestamp, (FLV_VIDEO_KEY_FRAME==video.keyframe) ? 1 : 0);
	}
}
