...
static void on_video_frame(salts_capture_t *capture,
...
int main(void) {
    ...
    /* List devices */
    salts_capture_device_t devices[16];
    int count = salts_capture_list_video_devices(devices, 16);
    ...
    salts_video_native_mode_t modes[256];
    const salts_video_native_mode_t *selected_mode = NULL;
    size_t mode_count = 0;
    printf("Opening device: %s\n", devices[0].name);

    if (salts_video_device_open(devices[0].id, &device) != SALTS_CAPTURE_OK) {
        printf("Failed to open video device.\n");
        return 1;
    }
    if (salts_video_device_list_modes(device, modes, 256, &mode_count) !=
        SALTS_CAPTURE_OK) {
    ...
    for (size_t i = 0; i < mode_count; ++i) {
        if (modes[i].format == SALTS_VIDEO_CAPTURE_FORMAT_MJPEG) {
    ...
    salts_capture_t *cap = NULL;
    int create_result = salts_video_device_create_capture(
        device, selected_mode, &cap);
    salts_video_device_close(device);
    if (create_result != SALTS_CAPTURE_OK || !cap) {
        printf("Failed to create capture instance.\n");
        return 1;
    }

    salts_video_capture_set_callback(cap, on_video_frame, NULL);

    if (salts_capture_start(cap) != 0) {
        printf("Failed to start capture.\n");
        salts_capture_destroy(cap);
        return 1;
    }
