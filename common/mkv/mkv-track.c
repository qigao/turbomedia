...
int mkv_write_track(struct mkv_ioutil_t* io, struct mkv_track_t* track)
{
    ...
    {
        size_t video_size = 2 + ebml_uint_length(track->u.video.width) +
                            2 + ebml_uint_length(track->u.video.height);
        if (track->u.video.alpha)
            video_size += 3 + ebml_uint_length((uint64_t)track->u.video.alpha);

        // Segment/Tracks/TrackEntry/Video
        mkv_buffer_write_master(io, 0xE0, video_size, 0);
        ...
        if (track->u.video.alpha)
            mkv_buffer_write_uint_element(io, 0x53C0, (uint64_t)track->u.video.alpha); // AlphaMode
    }
