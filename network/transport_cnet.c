...
static uint32_t transport_timeout(const turbo_transport_impl_t *transport) {
    return transport->config.read_timeout_ms > 0
        ? (uint32_t)transport->config.read_timeout_ms : TRANSPORT_CONNECT_TIMEOUT_MS;
}
...
static cnet_client_config transport_client_config(
    ...
    uint32_t connect_timeout = transport->config.connect_timeout_ms > 0
        ? (uint32_t)transport->config.connect_timeout_ms : TRANSPORT_CONNECT_TIMEOUT_MS;
    return (cnet_client_config){
        ...
        .write_timeout_ms = transport->config.write_timeout_ms > 0
            ? (uint32_t)transport->config.write_timeout_ms : TRANSPORT_CONNECT_TIMEOUT_MS,
...
turbo_transport_t *turbo_transport_create(const turbo_transport_config_t *config) {
    ...
    transport->config.host = transport_dup(config->host);
    transport->config.path = transport_dup(config->path);
...
static int connect_websocket(turbo_transport_impl_t *transport) {
    ...
    config.max_frame_bytes = TRANSPORT_MAX_BYTES;
    config.max_message_bytes = TRANSPORT_MAX_BYTES;
    config.max_buffered_input_bytes = TRANSPORT_MAX_BYTES;
    config.max_handshake_header_bytes = 32 * 1024;
    if (chttp_websocket_client_init(&transport->websocket, &config) != SALTS_OK) return -1;
    transport->websocket_initialized = 1;
    snprintf(uri, sizeof(uri), "%s://%s:%d%s",
             transport->config.use_tls ? "wss" : "ws", transport->config.host,
             transport->config.port, transport->config.path ? transport->config.path : "/");
    options.size = sizeof(options);
    options.uri = uri;
    options.timeout_ms = transport_timeout(transport);
    if (chttp_websocket_client_connect(&transport->websocket, &options, &status) != SALTS_OK)
        return -1;
    transport->connected = 1;
    transport_fire_event(transport, TURBO_TRANSPORT_EVENT_CONNECTED, NULL);
    return 0;
...
int turbo_transport_connect(turbo_transport_t *transport_ptr) {
    ...
    if (!transport || transport->connected) return transport ? 0 : -1;
    if (transport->config.type == TURBO_TRANSPORT_HTTP)
        return turbo_transport_connect_http(transport_ptr);
    if (transport->config.type == TURBO_TRANSPORT_WEBSOCKET) return connect_websocket(transport);
    if (transport->config.type == TURBO_TRANSPORT_UDP) {
        transport->connected = 1;
        transport_fire_event(transport, TURBO_TRANSPORT_EVENT_CONNECTED, NULL);
        return 0;
    }
    snprintf(uri, sizeof(uri), strchr(transport->config.host, ':')
                                      ? "%s://[%s]:%d" : "%s://%s:%d",
             transport->config.type == TURBO_TRANSPORT_TLS ? "tls" : "tcp",
             transport->config.host, transport->config.port);
    options.uri = uri;
    options.observer = (cnet_observer){cnet_state, cnet_receive_cb, transport, cnet_send_cb};
    options.tls = transport->config.type == TURBO_TRANSPORT_TLS
                      ? transport->config.tls
                      : NULL;
    if (cnet_connect(transport->client, &options, &transport->connection) != SALTS_OK) return -1;
...
int turbo_transport_disconnect(turbo_transport_t *transport_ptr) {
    ...
    if (transport->websocket_initialized)
        (void)chttp_websocket_client_close(&transport->websocket, 1000, NULL, 0,
                                           TRANSPORT_STOP_TIMEOUT_MS);
    else if (transport->client) (void)cnet_close(transport->client, transport->connection);
    transport->connected = 0;
    transport_fire_event(transport, TURBO_TRANSPORT_EVENT_DISCONNECTED, NULL);
    return 0;
}
...
int turbo_transport_send(turbo_transport_t *transport_ptr, const uint8_t *data, size_t size) {
    ...
    size_t events = 0;
    int status;
    if (!transport || !transport->connected || !data || size == 0) return -1;
    if (transport->websocket_initialized)
...
int turbo_transport_recv(turbo_transport_t *transport_ptr, uint8_t **data, size_t *size) {
    ...
    size_t events = 0;
    int status;
    if (!transport || !transport->connected || !data || !size) return -1;
...
int turbo_transport_is_connected(turbo_transport_t *transport) {
    return transport && ((turbo_transport_impl_t *)transport)->connected;
}

void turbo_transport_set_event_callback(turbo_transport_t *transport,
                                        turbo_transport_event_cb callback, void *user_data) {
    turbo_transport_impl_t *impl = (turbo_transport_impl_t *)transport;
    if (!impl) return;
...
int turbo_transport_get_connection(turbo_transport_t *transport,
                                   cnet_connection *connection) {
    turbo_transport_impl_t *impl = (turbo_transport_impl_t *)transport;
    if (!impl || !connection || !impl->client) return -1;
...
const char *turbo_transport_get_error(turbo_transport_t *transport) {
    turbo_transport_impl_t *impl = (turbo_transport_impl_t *)transport;
    if (impl && impl->config.type == TURBO_TRANSPORT_HTTP)
...
int turbo_transport_ws_send_text(turbo_transport_t *transport_ptr, const char *text) {
    turbo_transport_impl_t *transport = (turbo_transport_impl_t *)transport_ptr;
    if (!transport || !transport->connected || !text || !transport->websocket_initialized) return -1;
...
int turbo_transport_ws_recv(turbo_transport_t *transport_ptr, uint8_t **data,
    ...
    if (!transport || !data || !size || !is_text || !transport->websocket_initialized) return -1;
...
void turbo_transport_destroy(turbo_transport_t *transport_ptr) {
    ...
    if (!transport) return;
    if (transport->config.type == TURBO_TRANSPORT_HTTP) {
        turbo_transport_destroy_http(transport_ptr);
        return;
    }
    if (transport->connected) (void)turbo_transport_disconnect(transport_ptr);
    if (transport->websocket_initialized)
        (void)chttp_websocket_client_destroy(&transport->websocket, TRANSPORT_STOP_TIMEOUT_MS);
    if (transport->datagram_initialized) {
        (void)cnet_datagram_stop(&transport->datagram, TRANSPORT_STOP_TIMEOUT_MS);
        (void)cnet_datagram_destroy(&transport->datagram);
    }
