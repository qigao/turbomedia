#ifndef ROOM_SERVICE_IVR_CONTROL_ADAPTER_H
#define ROOM_SERVICE_IVR_CONTROL_ADAPTER_H

/**
 * @file ivr_control_adapter.h
 * @brief RoomService-side host adapter for the IVR CHTTP H1 WebSocket bridge.
 *
...
#define IVR_CONTROL_ADAPTER_MAX_WORKER_ACLS 32u

typedef struct ivr_control_adapter_s ivr_control_adapter_t;

typedef enum {
    IVR_CONTROL_WORKER_SYNCED = 1,
    IVR_CONTROL_WORKER_READY = 2,
    IVR_CONTROL_WORKER_DRAINING = 3,
    IVR_CONTROL_WORKER_EXPIRED = 4,
    ...
    IVR_CONTROL_WORKER_RECONCILING = 5
} ivr_control_worker_state_t;
...
typedef struct {
    ...
    ivr_control_worker_state_t state;
...
} ivr_control_worker_snapshot_t;

/* One configured worker authorization entry (from RoomService [control_ws.workers]).
...
} ivr_control_media_observer_ops_t;

typedef struct {
    void *context;
    /* Called on the CHTTP H1 WebSocket bridge owner thread after route and worker-epoch
...
} ivr_control_inventory_observer_ops_t;

typedef struct {
    const char *bind_host; /* WebSocket bind address; NULL = "127.0.0.1" */
    int bind_port;         /* <= 0 disables the control WebSocket endpoint */
    ...
    const ivr_control_worker_acl_entry_t *worker_acls; /* NULL = legacy allow-all */
...
} ivr_control_adapter_config_t;

typedef enum {
    IVR_CONTROL_ASSIGNMENT_PENDING = 1,
    IVR_CONTROL_ASSIGNMENT_ACCEPTED = 2,
    IVR_CONTROL_ASSIGNMENT_REJECTED = 3,
    IVR_CONTROL_ASSIGNMENT_RELEASING = 4,
    IVR_CONTROL_ASSIGNMENT_RECOVERING = 5
} ivr_control_assignment_state_t;
...
typedef struct {
    ...
    ivr_control_assignment_state_t state;
    int status_code;
    char error_code[64];
...
} ivr_control_adapter_stats_t;

/* Create the adapter. When bind_port <= 0 no control WebSocket endpoint is created and the
...
/* Apply one decoded IVR command to the authoritative aggregate and fill the
   ...
   logic: no CHTTP H1 WebSocket is touched, so it is unit-testable without a peer. */
...
#endif /* ROOM_SERVICE_IVR_CONTROL_ADAPTER_H */
