...
static const char IRIS_ORM_CREATE_POSTGRESQL_SQL[] =
    ...
    "namespace text not null,record_key bytea not null,revision bigint not "
    "null,"
...
static int iris_yaml_mapping_keys_valid(const cyaml_doc_t *document,
                                        const cyaml_node_t *mapping,
                                        const char *const *allowed,
                                        size_t allowed_count) {
    size_t count;
    size_t i;
    if (!document || !iris_yaml_mapping(mapping) || !allowed)
        return 0;
    ...
    for (i = 0u; i < count; ++i) {
        ...
        size_t j;
        int matched = 0;
        if (!key_node || iris_yaml_node_type(key_node) != CYAML_SCALAR) {
            return 0;
        }
...
static int iris_parse_size(const cyaml_doc_t *document,
                           const cyaml_node_t *mapping, const char *key,
                           size_t default_value, size_t minimum, size_t maximum,
                           size_t *out) {
    char *text;
...
static int iris_orm_parse_config(const char *yaml_path,
    ...
    if (!iris_yaml_mapping_keys_valid(document, root, root_keys,
                                      sizeof(root_keys) /
                                          sizeof(root_keys[0]))) {
        iris_orm_write_error(error, error_capacity,
    ...
    channels =
        iris_yaml_mapping(root) ? cyaml_get(document, root, "channels") : NULL;
    ...
    if (!iris_yaml_mapping_keys_valid(document, channel, channel_keys,
                                      sizeof(channel_keys) /
                                          sizeof(channel_keys[0]))) {
        iris_orm_write_error(error, error_capacity,
    ...
    if (!iris_yaml_mapping_keys_valid(document, channel_config,
                                      postgresql_config_keys,
    ...
    config->pg_dbname = iris_yaml_string(document, channel_config, "dbname");
    config->pg_service = iris_yaml_string(document, channel_config, "service");
    config->pg_connect_timeout =
        iris_yaml_string(document, channel_config, "connect_timeout");
    config->pg_sslmode = iris_yaml_string(document, channel_config, "sslmode");
    config->pg_sslrootcert =
        iris_yaml_string(document, channel_config, "sslrootcert");
    config->pg_sslcert = iris_yaml_string(document, channel_config, "sslcert");
    config->pg_sslkey = iris_yaml_string(document, channel_config, "sslkey");
    if ((!config->pg_host && !config->pg_port && !config->pg_user &&
         !config->pg_password && !config->pg_dbname && !config->pg_service &&
         !config->pg_connect_timeout && !config->pg_sslmode &&
         !config->pg_sslrootcert && !config->pg_sslcert &&
         !config->pg_sslkey) ||
...
static int iris_orm_current_revision(iris_orm_store_owner_t *owner,
    ...
    int rc;
    if (!revision || !exists || !item_bytes)
        return SALTS_EINVAL;
    values[0] = orm_text(owner->namespace_name);
    values[1] = orm_blob(key, key_size);
    rc = iris_orm_execute_raw(owner, transaction, postgresql_sql, values, 2u,
                              &result);
    if (rc != SALTS_OK)
        return rc;
    orm_error_init(&error);
...
static int iris_orm_record_count(iris_orm_store_owner_t *owner,
    ...
    int rc = iris_orm_execute_raw(owner, transaction, postgresql_sql,
...
static int iris_orm_total_bytes(iris_orm_store_owner_t *owner,
    ...
    if (!total_bytes)
        return SALTS_EINVAL;
    rc = iris_orm_execute_raw(owner, transaction, postgresql_sql,
                              &namespace_value, 1u, &result);
    if (rc != SALTS_OK)
        return rc;
    orm_error_init(&error);
...
static int iris_orm_scan(void *context, iris_record_visit_fn visit,
    ...
    rc = iris_orm_execute_raw(owner, NULL, postgresql_sql, &namespace_value, 1u,
                              &result);
    if (rc != SALTS_OK)
        return rc;
    orm_error_init(&error);
...
static int iris_orm_validate_mutations(const iris_orm_store_owner_t *owner,
    ...
    for (i = 0u; i < mutation_count; ++i) {
        ...
        for (j = i + 1u; j < mutation_count; ++j) {
            if (mutation->key_size == mutations[j].key_size &&
                memcmp(mutation->key, mutations[j].key, mutation->key_size) ==
                    0) {
                return SALTS_EINVAL;
            }
        }
...
static int iris_orm_mutate(iris_orm_store_owner_t *owner,
    ...
    static const char insert_postgresql_sql[] =
        "insert into "
        "iris_record_store(namespace,record_key,revision,record_value) "
        "values($1,$2,$3,$4)";
    ...
    static const char delete_postgresql_sql[] =
        "delete from iris_record_store where namespace=$1 and record_key=$2 "
        "and revision=$3";
    ...
    if (mutation->kind == IRIS_RECORD_DELETE) {
        ...
        rc = iris_orm_execute_raw(owner, transaction, delete_postgresql_sql,
                                  values, 3u, &result);
        ...
        rc = iris_orm_execute_raw(owner, transaction, insert_postgresql_sql,
                                  values, 4u, &result);
    } else {
        ...
        rc = iris_orm_execute_raw(owner, transaction, update_postgresql_sql,
                                  values, 5u, &result);
    }
    if (rc != SALTS_OK)
        return rc;
...
static int iris_orm_commit(void *context,
    ...
    if (rc != SALTS_OK)
        return rc;
    orm_error_init(&error);
    status = orm_transaction_begin(
        owner->connection, ORM_ISOLATION_SERIALIZABLE, &transaction, &error);
    if (status != ORM_STATUS_OK)
        return iris_orm_status(status);
    ...
    for (i = 0u; i < mutation_count; ++i) {
        ...
        rc = iris_orm_current_revision(owner, transaction, mutations[i].key,
                                       mutations[i].key_size, &revision,
                                       &exists, &old_item_bytes);
        if (rc != SALTS_OK)
            goto rollback;
        if ((exists && revision != mutations[i].expected_revision) ||
            (!exists &&
             mutations[i].expected_revision != IRIS_RECORD_REVISION_ABSENT)) {
            rc = SALTS_EBUSY;
            goto rollback;
        }
        if (!exists && mutations[i].kind == IRIS_RECORD_PUT)
            inserts++;
        if (exists && mutations[i].kind == IRIS_RECORD_DELETE)
            deletes++;
...
static void iris_orm_add_option(orm_option_t *options, size_t capacity,
                                ...
                                const char *value) {
    if (!options || !count || *count >= capacity || !keyword || !value)
        return;
...
iris_orm_store_owner_t *iris_orm_store_owner_create(const char *yaml_path,
                                                    const char *channel_name,
                                                    char *error,
                                                    size_t error_capacity) {
    ...
    if (!iris_orm_parse_config(yaml_path, channel_name, &parsed, error,
                               error_capacity)) {
        return NULL;
    }
    ...
    iris_orm_add_option(options, 11u, &option_count, "user", parsed.pg_user);
    ...
    if (iris_orm_execute_raw(owner, NULL, IRIS_ORM_CREATE_POSTGRESQL_SQL, NULL,
                             0u, &result) != SALTS_OK) {
        iris_orm_write_error(error, error_capacity,
