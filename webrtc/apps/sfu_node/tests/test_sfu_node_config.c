...
spec("SFU node TOML configuration") {
    ...
    it("tenant quota requires TLS and signed auth") {
        sfu_node_app_config_t config;

        sfu_node_app_config_init(&config);
        config.tenant_quota_capacity = 8;
        check_equal(sfu_node_app_config_validate(&config), -1);
        ...
        config.control_token = "legacy-control-token";
        check_equal(sfu_node_app_config_validate(&config), -1);
        config.control_token = NULL;
        config.media_access_token = "legacy-media-token";
        check_equal(sfu_node_app_config_validate(&config), -1);
