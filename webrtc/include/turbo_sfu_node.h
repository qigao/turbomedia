...
TURBO_MEDIA_API turbo_sfu_node_t *turbo_sfu_node_create(
    const turbo_sfu_node_config_t *config);
TURBO_MEDIA_API void turbo_sfu_node_destroy(turbo_sfu_node_t *node);

TURBO_MEDIA_API int turbo_sfu_node_attach_room(turbo_sfu_node_t *node, const char *room_id,
                                         int max_participants);
TURBO_MEDIA_API int turbo_sfu_node_detach_room(turbo_sfu_node_t *node, const char *room_id);
TURBO_MEDIA_API int turbo_sfu_node_force_close_room(turbo_sfu_node_t *node, const char *room_id);

TURBO_MEDIA_API int turbo_sfu_node_add_session(turbo_sfu_node_t *node, const char *room_id,
                                         const char *participant_id,
                                         const char *session_id,
                                         turbo_peer_connection_t *pc);
TURBO_MEDIA_API int turbo_sfu_node_bind_session_pc(turbo_sfu_node_t *node,
                                             const char *room_id,
                                             const char *participant_id,
                                             const char *session_id,
                                             turbo_peer_connection_t *pc);
TURBO_MEDIA_API int turbo_sfu_node_remove_session(turbo_sfu_node_t *node, const char *room_id,
                                            const char *session_id);

TURBO_MEDIA_API int turbo_sfu_node_register_published_track(
    ...
    int layer_count);
TURBO_MEDIA_API int turbo_sfu_node_unregister_published_track(
    turbo_sfu_node_t *node, const char *room_id, const char *track_id);
TURBO_MEDIA_API int turbo_sfu_node_set_receiver_bandwidth(turbo_sfu_node_t *node,
                                                    const char *room_id,
                                                    const char *participant_id,
                                                    int bandwidth_bps);
TURBO_MEDIA_API int turbo_sfu_node_set_track_subscription(
...
TURBO_MEDIA_API int turbo_sfu_node_apply_track_subscription(
    turbo_sfu_node_t *node, const char *room_id,
    const turbo_sfu_node_track_subscription_t *subscription);
TURBO_MEDIA_API int turbo_sfu_node_set_participant_packet_callback(
    turbo_sfu_node_t *node, const char *room_id, const char *participant_id,
    turbo_sfu_node_packet_cb on_packet, void *user_data);
TURBO_MEDIA_API int turbo_sfu_node_set_participant_keyframe_callback(
    turbo_sfu_node_t *node, const char *room_id, const char *participant_id,
    turbo_sfu_node_keyframe_request_cb on_keyframe_request, void *user_data);
TURBO_MEDIA_API int turbo_sfu_node_forward_packet(turbo_sfu_node_t *node, const char *room_id,
                                            const char *sender_participant_id,
                                            const uint8_t *packet, size_t len);

TURBO_MEDIA_API int turbo_sfu_node_get_room_stats(turbo_sfu_node_t *node,
                                            const char *room_id,
                                            turbo_sfu_node_room_stats_t *stats);
TURBO_MEDIA_API int turbo_sfu_node_get_participant_stats(
    turbo_sfu_node_t *node, const char *room_id, const char *participant_id,
    turbo_sfu_node_participant_stats_t *stats);
TURBO_MEDIA_API int turbo_sfu_node_get_track_subscription(
    turbo_sfu_node_t *node, const char *room_id, const char *receiver_participant_id,
    const char *track_id, turbo_sfu_node_track_subscription_t *subscription);
TURBO_MEDIA_API void turbo_sfu_node_get_stats(turbo_sfu_node_t *node,
