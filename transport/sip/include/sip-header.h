...
#define SIP_VEC_IMPLEMENT(name, initial_capacity)                                                 \
  void name##s_init(struct name##s_t *items) {                                                    \
    if (vec_init_bytes(&items->items, sizeof(struct name##_t), CMETA_ALIGNOF(struct name##_t), SIZE_MAX / sizeof(struct name##_t)) != STL_OK ||                    \
        vec_reserve(&items->items, (initial_capacity)) != STL_OK)                         \
      abort();                                                                                    \
  }                                                                                               \
  void name##s_free(struct name##s_t *items) {                                                    \
    size_t index;                                                                                 \
    for (index = 0U; index < vec_size(&items->items); ++index)                              \
      name##_free((struct name##_t *)vec_at(&items->items, index));                         \
    vec_destroy(&items->items);                                                             \
  }                                                                                               \
  int name##s_push(struct name##s_t *items, struct name##_t *item) {                             \
    return vec_push(&items->items, item) == STL_OK ? 0 : -ENOMEM;                         \
  }                                                                                               \
  struct name##_t *name##s_get(const struct name##s_t *items, int index) {                       \
    if (index < 0) return NULL;                                                                   \
    return (struct name##_t *)vec_at((vec_t *)&items->items, (size_t)index);          \
  }                                                                                               \
  int name##s_count(const struct name##s_t *items) {                                             \
    return (int)vec_size(&items->items);                                                    \
  }
...
{
	vstr scheme;
	vstr host; // userinfo@host:port
	
	struct sip_params_t parameters;
	vstr transport; // udp/tcp/sctp/tls/other
	vstr method;
	vstr maddr; // the server address to be contacted for this user, overriding any address derived from the host field
	vstr user; // phone/ip
	int ttl;
	int lr;
...
struct sip_via_t
{
	...
	vstr version;
	vstr transport;
	vstr host; // sent-by host:port

	// parameters
	vstr branch; // token
	vstr maddr; // host
	vstr received; // IPv4address / IPv6address
	int ttl; // 0-255
	int rport; // 0-not found, -1-no-value, other-value
