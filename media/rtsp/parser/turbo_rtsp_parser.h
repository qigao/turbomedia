...
CXX_C_API int turbo_rtsp_parse_request(
    const char *buffer,
    size_t buffer_len,
    size_t *consumed,
    turbo_rtsp_request_t *request);

CXX_C_API int turbo_rtsp_parse_response(
    const char *buffer,
    size_t buffer_len,
    size_t *consumed,
    turbo_rtsp_response_view_t *response);

CXX_C_API int turbo_rtsp_parse_transport(
    const char *transport,
    size_t transport_len,
    turbo_rtsp_transport_spec_t *spec);

CXX_C_API int turbo_rtsp_parse_rtp_info(
    const char *rtp_info,
    size_t rtp_info_len,
...
CXX_C_API int turbo_rtsp_format_request(
    char *buffer,
    size_t buffer_size,
    const turbo_rtsp_request_message_t *request);

CXX_C_API int turbo_rtsp_format_response(
    char *buffer,
    size_t buffer_size,
    uint32_t cseq,
